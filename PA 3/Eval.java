...
public class Eval {
	...
	static double value_roll_hand(State s, int depth) {
		...
		// Set for dice face for 3 types of landing 
		for(DieFace face1: DieFace.values()){
			for(DieFace face2: DieFace.values()){ 
				for (DieFace face3: DieFace.values()) { 
					//calculating probability 
					double this_roll_pro = s.rollProb(face1,  face2, face3);
				
					s.rollInPlace(face1,face2,face3);
					
					double rollvalue = Eval.value_rolled_hand(s, depth);
					
				// Sum the total runs
					val = val + (rollvalue = this_roll_pro); 
	...
	static public double heuristic(State s) {
		...
		// calculate the difference between # of
		//brains that computer has and user has for win
		int forWin = State.brains_to_win - s.brains_collected;
		

		double feet = 0;
	
		// brain collected total - eaten
		double user= s.comp_brains_eaten + s.brains_collected - s.user_brains_eaten;
